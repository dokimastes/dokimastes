...
pub fn run(args: Args) -> Result<ExitCode> {
    ...
    if let Some(text) = &args.first_agent_run {
        let first_run = parse_iso_date(text)
            .with_context(|| format!("--first-agent-run {text:?} is not a YYYY-MM-DD date"))?;
        if now >= first_run + DAY {
            baseline.refusals.push(format!(
...
pub fn compute(repo: &str, h: &History, window_days: u32, now: i64) -> Baseline {
    ...
    let mut lead_hours: Vec<f64> = h
        ...
        .collect();
    let linear = h.merges.is_empty();
    let merge_note = if linear {
        " — no merge commits in the window: a linear or squash-merged history leaves none, so pull-request figures need the platform API"
    } else {
        ""
    };
    ...
    let pairs = vec![
        Pair {
            ...
            counter: Metric::recovered(
                ...
                "% of added lines",
                "lines added in the window and deleted again in the same file within 14 days, at file granularity — an upper bound on line-level churn",
            ),
        },
        Pair {
            throughput: Some(Metric::recovered("merges per week", Some(h.merges.len() as f64 / weeks), "merges/week", &format!("merge commits on the first-parent line, divided by weeks{merge_note}"))),
            counter: Metric::recovered("revert commits", Some(reverts as f64), "commits", "commits whose subject starts with \"Revert \""),
        },
        Pair {
            throughput: Some(Metric::recovered("median merge size", history::median(&mut sizes), "lines", &format!("lines added plus deleted per merge against its first parent{merge_note}"))),
        ...
        Pair {
            throughput: Some(Metric::recovered("median lead time", history::median(&mut lead_hours), "hours", &format!("oldest merged commit to merge commit{merge_note}"))),
            counter: Metric::absent("merges with no review", "% of merges", Status::PlatformApi, "review state lives on the platform, not in git"),
        },
        Pair {
            throughput: Some(Metric::recovered("releases per week", Some(h.release_tags.len() as f64 / weeks), "tags/week", "v* tags created in the window, divided by weeks — a proxy for deploys; deploy records themselves are not in git")),
...
mod tests {
    ...
    fn figures_are_paired_and_computed() {
        let b = compute("r", &history(), 70, 70 * DAY);
    ...
    #[test]
    fn a_linear_history_says_so() {
        let mut h = history();
        h.merges.clear();
        let b = compute("r", &h, 70, 70 * DAY);
        assert!(metric(&b, "merges per week")
            .method
            .contains("platform API"));
        assert_eq!(metric(&b, "median merge size").value, None);
    }
